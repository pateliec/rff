...
$(document).ready(function () {
	$("#loading").show();
    $.ajax({
        url: "<?php echo site_url(); ?>/processbooking/packageBookingAjax",
        type: "get",
...
function getDeparture(date, isReturn)
{
	if(isReturn == "true")
	{
		alert('You can not change return date in case of package booking');
		return;
	}
	$("#loading").show();
    $.ajax({
        url: "<?php echo site_url(); ?>/processbooking/packageBookingAjax?departDate="+date+"&isReturn="+isReturn,
        type: "get",
        success: function (response) {
            $("#booking-section").html(response);
			$("#loading").hide();
